...
export abstract class BaseScraperStrategy {
  ...
  protected parseProgressPrefix(line: string) {
    if (
      line.includes("API rate limit exceeded") ||
  ...
  protected checkLimits(child: ChildProcess) {
    ...
    if (
      this.limits?.stopAfterCompleted &&
      this.downloadedCount >= this.limits.stopAfterCompleted
    ) {
      console.log(
        `[ScraperStrategy] Reached download limit of ${this.limits.stopAfterCompleted}. Stopping.`,
      );
    ...
    if (
      this.limits?.stopAfterPosts &&
      this.postsProcessed >= this.limits.stopAfterPosts
    ) {
      console.log(
        `[ScraperStrategy] Reached post limit of ${this.limits.stopAfterPosts}. Waiting for finalization...`,
      );
      this.intentionalStop = true;
      this.stopInitiated = true;
