...
export async function GET(
  ...
  if (platform === "twitter") {
    const user = await db
      .select({ profileImage: twitterUsers.profileImage })
      .from(twitterUsers)
    ...
    const user = await db
      .select({ profileImage: pixivUsers.profileImage })
      .from(pixivUsers)
  ...
  if (!avatarUrl) {
    return new NextResponse("User not found or no avatar URL", {
      status: 404,
    });
  }
  ...
  try {
    if (!avatarUrl.startsWith("http")) {
      console.warn(
        `[API] Invalid or non-remote URL for ${platform}/${userId}. DB value: ${avatarUrl}`,
      );
      return new NextResponse(`Avatar not found and no remote URL available`, {
        status: 404,
      });
    }
    ...
    await avatarRequestQueue.enqueue(async () => {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch remote avatar: ${response.status} ${response.statusText}`,
        );
      }
