...
interface PostPageClientProps {
  ...
  /** Post-level fallback for author/platform info (#76 review finding 2):
   *  text-only posts have no media rows to derive these from. */
  postPlatformUser?: PlatformUser | null;
  postPlatformDetails?: {
    twitter?: TwitterDetails | null;
    pixiv?: PixivDetails | null;
    gelbooru?: GelbooruDetails | null;
  } | null;
  pageSize: number;
  loopVideos: boolean;
...
export default function PostPageClient(props: PostPageClientProps) {
  ...
  const platformUser = first?.platformUser ?? props.postPlatformUser ?? null;
  // Media-derived platform details first; post-level rows as fallback so
  // text-only posts still show their platform stats (#76 review finding 2).
  const twitterDetails =
    (first ? getPlatformDetails<TwitterDetails>(first, "twitter") : null) ??
    props.postPlatformDetails?.twitter ??
    null;
  const pixivDetails =
    (first ? getPlatformDetails<PixivDetails>(first, "pixiv") : null) ??
    props.postPlatformDetails?.pixiv ??
    null;
  const gelbooruDetails =
    (first
      ? getPlatformDetails<GelbooruDetails>(first, "gelbooruv02")
      : null) ??
    props.postPlatformDetails?.gelbooru ??
    null;
        ...
        <div className={styles.authorRow}>
          ...
          <div className={styles.authorMeta}>
            <span className={styles.authorName}>
              {platformUser?.name || platformLabel}
            </span>
          ...
          {/* Rendered independently of platformUser (review finding 3):
              Gelbooru/generic posts have a source URL but no platform user. */}
          {post.url && (
            <a
              href={post.url}
              target="_blank"
