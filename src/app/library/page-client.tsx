...
export default function LibraryPageClient({
  ...
  initialScanStatus: ScanStatus | null;
}) {
  const [scanning, setScanning] = useState(
    initialScanStatus?.status === "running",
  );
          ...
          <div className={styles.scanStats}>
            <div className={styles.statsRow}>
              <div>
                ...
                <span
                  className={`${styles.badge} ${styles[`status-${scanStatus.status}`] || ""}`}
                >
                  {scanStatus.status}
                </span>
                {scanStatus.scanType && (
              ...
              {scanStatus.status !== "running" && (
                <>
                  ...
                  <div>
                    ...
                    <span className={styles.resultAdded}>
                    ...
                    {" / "}
                    <span className={styles.resultDeleted}>
                      -{scanStatus.filesDeleted}
                    </span>
                  </div>
                </>
              )}
        ...
        <div className={styles.dangerActions}>
          <button
            ...
            onClick={async () => {
              if (
                confirm(
                  "ARE YOU SURE? This will delete ALL data, including the database and gallery-dl archives. This cannot be undone.",
                )
              ) {
          ...
          <button
            ...
            onClick={async () => {
              if (
                confirm(
                  "Delete all cached avatars? They will be re-downloaded as needed.",
                )
              ) {
                const btn = document.activeElement as HTMLButtonElement;
                if (btn) {
          ...
          <button
            ...
            onClick={async () => {
              if (
                confirm("Delete ALL downloaded files? This cannot be undone.")
              ) {
                const btn = document.activeElement as HTMLButtonElement;
                if (btn) {
                  btn.disabled = true;
